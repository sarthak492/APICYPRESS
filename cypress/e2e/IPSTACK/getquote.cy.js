describe('Contact Form Page', () => {
    ...
    it('2. Test to verify the presence of first name field', () => {
        cy.get('#first_name').should('be.visible');
    });

    it('3. Test to verify last name field', () => {
        cy.get('#last_name').should('be.visible');
    });

    it('4. Test to verify the presence of the message textarea', () => {
        cy.get(':nth-child(3) > input').should('be.visible');
    });

    it('5. Test to verify the presence of country code', () => {
        cy.get('#country_code').should('be.visible');
    });

    it('6. Test to verify the subject dropdown is required', () => {
        cy.get(':nth-child(6) > select').should('be.visible')
    });

    it('7. Test to verify the message textarea is required', () => {
        cy.get(':nth-child(7) > textarea').should('be.visible');
    });

    it('8. Test to check the presence of company information', () => {
        cy.contains('Apilayer Data Products GmbH').should('be.visible');
    });

    it('9. Test to verify the company information address', () => {
        cy.contains('Oppolzergasse 6/1/4').should('be.visible');
    });

    it('10. Test to click on the FAQ link and verify it navigates to the FAQ section', () => {
        cy.contains('FAQ section').click();
        cy.url().should('include', 'https://ipstack.com/faq');
        cy.visit('https://ipstack.com/contact');
    });

    it('11. Test to display validation errors when submitting empty form', () => {
        cy.get('.line.submit > .submit').click();
        cy.get('.alerts').should('be.visible').and('contain.text', 'Please enter your first name.');
    });

    it('12. Test to display error for last name after filling first name', () => {
        cy.get('#first_name').type('John');
        cy.get('.line.submit > .submit').click();
        cy.get('.alerts').should('be.visible').and('contain.text', 'Please enter your last name.');
    });

    it('13. Test to display error for email after filling last name', () => {
        cy.get('#last_name').type('Doe');
        cy.get('.line.submit > .submit').click();
        cy.get('.alerts').should('be.visible').and('contain.text', 'Please provide a valid e-mail address.');
    });

    it('14. Test to display error for country code after filling email', () => {
    ...
    it('15. Test to display error for Subject after filling country code', () => {
    ...
    it('16. Test to display error for message', () => {
    ...
    it('17. Test to display error for  after message', () => {
        cy.get(':nth-child(6) > select').select('Sales');
        cy.get('.line.submit > .submit').click();
        cy.get('.alerts').should('be.visible').and('contain.text', 'Please compose a message of at least 10 characters.');
    });

    it('18. Test to check the number of characters more than 10', () => {
    ...
    it('19. Test to navigate to the doc page after clicking the explore docs', () => {
        cy.get('.docs').click();
        cy.visit('https://ipstack.com/documentation');
        cy.visit('https://ipstack.com/contact');
    })

    it('20. Test to navigate to the doc page after clicking the explore docs', () => {
